...
async function generateDashboard(type, subject=''){
    ...
    // Generate left graph 
    generateGraph(leftGraphTitle, colourResponseObj, responseObj, timesList, totalTime, "graph-left");
...
async function generateGoalsGraph(ID){
    ...
    for(let i=0; i < goalsDict.length; i++){
        ...
        const subjectTimeResponse = await fetch('/get/times/subject/'+ goalsDict[i].goalSubject);
        const subjectTimeResponseObj = await subjectTimeResponse.text();

        let fractionalDifference = (Number(subjectTimeResponseObj)/Number(goalsDict[i].goalDuration));
